...
Note: The live site depends on our AWS RDS PostgreSQL instance. If the RDS security group is not open/allowing connections (or the database is paused/unreachable), the deployed backend cannot query the database and the live site may fail to load data or return errors.
